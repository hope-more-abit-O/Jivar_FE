...
export default function KanbanProject() {
  ...
  const fetchProjectById = async (projectId) => {
    try {
      const response = await axios.get(
        `http://localhost:5287/api/Project/${projectId}?includeRole=true&includeSprint=true&includeTask=true`
      );
      if (response.data) {
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      Object.keys(createIssueRefs.current).forEach((column) => {
        if (
          createIssueRefs.current[column] &&
          !createIssueRefs.current[column].contains(event.target) &&
          !event.target
            .closest("button")
            ?.contains(createIssueRefs.current[column])
        ) {
  ...
  const handleSearchUser = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5287/api/v1/account/info/user/${searchAccountId}`
      );
  ...
  const handleAddUserToProject = async () => {
    if (!searchedUser || !role) {
      alert(
        "Please select a user and assign a role before adding to the project."
      );
      return;
    }
    ...
    if (!searchedUser.accountId) {
      alert(
        "Error: Account ID is missing. Please ensure the user is selected correctly."
      );
      return;
    }
    ...
    try {
      ...
      if (response.status === 200) {
        setProject((prev) => ({
          ...
          project_roles: [
            ...prev.project_roles,
            {
              account_id: searchedUser.accountId,
        ...
        alert(
          `Success: User ${searchedUser.name} has been added to the project as ${role}.`
        );
      ...
      if (error.response) {
        ...
        alert(
          "An error occurred. Please check your network connection or try again later."
        );
      }
    }
  };
  ...
  const handleCreateSprintSubmit = async () => {
    try {
      await axios.post(
        `http://localhost:5287/api/v1/sprint?projectId=${projectId}`,
        newSprintData
      );
      setIsSprintDialogOpen(false);
      setNewSprintData({ name: "", startDate: "", endDate: "" });
  ...
  const fetchUploadedFiles = async () => {
    try {
      const accessToken = Cookies.get("accessToken");
      const response = await axios.get(
        "http://localhost:5287/api/v1/document",
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
  ...
  const handleRemoveMember = async (accountId) => {
    try {
      ...
      if (response.status === 200) {
        ...
        setProject((prev) => ({
          ...prev,
          project_roles: prev.project_roles.filter(
  ...
  const renderAllMembers = () => {
    ...
    const uniqueRoles = Array.from(
      new Map(
        project.project_roles.map((role) => [role.account_id, role])
      ).values()
    );
                ...
                {currentUser?.role === "Owner" && role.role !== "Owner" && (
                  <div className="flex items-center gap-2">
                    <select
                      ...
                      value={role.role}
                      onChange={(e) =>
                        handleUpdateRole(role.account_id, e.target.value)
                      }
                    >
  ...
  const handleCreateTask = async (sprintId, title, column) => {
    ...
    try {
      ...
      console.error(
        "Error creating or updating task:",
        error.response?.data || error.message
      );
    }
  };
            ...
            <div className="space-y-4">
              <div className="border-t-2">
                <Typography
                  variant="small"
                  color="blue-gray"
                  className="font-bold uppercase tracking-wide mb-2 mt-2"
                >
                  PLANNING
                </Typography>
              </div>
              <Button
                variant="text"
                color="blue"
                className="flex items-center justify-start normal-case bg-blue-50"
                fullWidth
              >
          ...
          <div className="p-6">
              ...
              <div className="flex justify-end ">
                <div className="flex items-center space-x-2 ">
                  ...
                  <Button
                    variant="outlined"
                    size="sm"
                    className="normal-case pe-3 flex"
                    color="blue-gray"
                  >
                    None
                      ...
                      {project.project_roles.map((role) => (
                        ...
                        >
                          ...
                          <span className="text-gray-700">
                            {role.username || "Unknown"} - {role.role}
                          </span>
                        </li>
                      ))}
                    </ul>
            ...
            {!selectedSprintId ? (
              ...
              <>
                ...
                <Button
                  className="ms-4 mb-4 bg-blue-500 text-white px-4 py-2 rounded"
                  onClick={() => {
                    const projectId = Cookies.get("projectId");
                    navigate(`/jivar/backlog?projectId=${projectId}`);
                  }}
                >
                  Create Backlog
                </Button>
